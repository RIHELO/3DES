...
public static int readFile(String filename, long maxval, byte[] buffer) throws IOException {
    try (FileInputStream file = new FileInputStream(filename)) {
        int size = file.read(buffer, 0, (int) maxval);
        return size;
    }
}
...
int Encryption( byte[] Buffer,  byte[] KEY, byte []out, int mode) {  
  ...
  do{     
       ...
       G +=8; 
   }while( G < Buffer.length-8 );
  return G;
}

public static void writeFile(String file, byte[] buffer, int size) {
...
public void doall(String[] args) {
    ...
    System.out.println(option == 0 ? "Encryption Mode\n" : "Decryption Mode\n");
    ...
    byte[] O2  = new byte[(int) fileSize];
    byte[] O3  = new byte[(int) fileSize];

    try (FileInputStream fileInputStream = new FileInputStream(file)) {
        int z = fileInputStream.read(INPUT, 0, INPUT.length);
        if (option == 0) {
             Q = Encryption(INPUT, K1, OUT, 0);
             Q = Encryption(OUT, K2, O2, 1);
             Q = Encryption(O2, K3, O3, 0);
             writeFile(args[2], O3, Q);
         } else {
