...
public class TripleDes {
    ...
    public void readFile(String filename, long maxval, byte[] buffer) throws IOException {
        try (FileInputStream file = new FileInputStream(filename)) {
            file.read(buffer, 0, (int) maxval);
        }
    }
    ...
    public void tobinary(byte[] line, int num, byte[] out) {
        int k = 0;
        for (int i = 0; i < num; i++) {
            int tmp = line[i] & 0xFF; // treat byte as unsigned
            for (int j = 7; j >= 0; j--) {
                out[k + j] = (byte) (tmp & 0x01);
                tmp >>= 1;
            }
            k += 8;
        }
    ...
    public void Copy(byte[] src, byte[] dest, int destOffset, int num) {
        if (num >= 0) System.arraycopy(src, 0, dest, destOffset, num);
    }
    ...
    public void LeftCShift(byte[] key, int i, byte[] out) {
        ...
        System.arraycopy(out, 0, key, 0, 56);
    }
    ...
    public void GetRight(byte[] a, byte[] out) {
        System.arraycopy(a, 32, out, 0, 32);
    }
    ...
    public byte GetBin(byte a, byte b) {
        if ((a == 0) && (b == 0))
            return 0;
        else if ((a == 0) && (b == 1))
            return 1;
    ...
    public void SubstitutionBox(byte[] a, byte[] out) {
        ...
        int aa = (S[0][Z[0]][Y[0]] << 4) | S[1][Z[1]][Y[1]];
        R[0] = (byte) aa;

        int bb = (S[2][Z[2]][Y[2]] << 4) | S[3][Z[3]][Y[3]];
        R[1] = (byte) bb;
        int cc = (S[4][Z[4]][Y[4]] << 4) | S[5][Z[5]][Y[5]];
        R[2] = (byte) cc;
        int dd = (S[6][Z[6]][Y[6]] << 4) | S[7][Z[7]][Y[7]];
        R[3] = (byte) dd;
        tobinary(R, 4, out);
    }
    ...
    public void ConvData(byte[] a, byte[] out, int outOffset) {
        for (int i = 0, k = 0; i < 8; i++, k += 8) {
            out[outOffset + i] = 0;
            for (int j = 0; j < 8; j++) {
                int tmp = out[outOffset + i] | a[k + j];
                out[outOffset + i] = (byte) tmp;
                if (j != 7) {
                    int tmp2 = out[outOffset + i];
                    tmp2 = tmp2 << 1;
                    out[outOffset + i] = (byte) tmp2;
                }
            }
        }
    }
    ...
    int Encryption(byte[] Buffer, byte[] KEY, byte[] out, int mode) {
        ...
        int G;
        for (G = 0; G + 8 <= Buffer.length; G += 8) {
            byte[] slice = Arrays.copyOfRange(Buffer, G, G + 8);
            tobinary(slice, 8, BLOCK);        //[OK]
            IniPermutation(BLOCK, Data);          //[OK]
            Permutation1(KEY, key);               //[OK]
            GetLeft(Data, Left);                  //[OK]
            GetRight(Data, Right);                //[OK]
            for (int i = 0; i < 16; i++) {
                Copy(Right, Temp, 0, 32);            //[OK]
    ...
    public void writeFile(String file, byte[] buffer) {
        ...
        try (FileOutputStream fos = new FileOutputStream(file)) {
            ...
            System.err.printf("ERROR: cannot save file > %s\n", file);
            e.printStackTrace();
            System.exit(1);
        }
    }
    ...
    public void Info() {
        ...
        des <options> <input file> <output file> <key file1> <key file2> <key file3>
        options:
        0  -->e: encryption
        1  -->d: decryption
        Examples:
    ...
    public void clean(byte[] k1, byte[] k2, byte[] k3, byte[] in) {
        int i;
    ...
    public byte[] addPadding(byte[] input) {
        ...
        if (padLength == 0) padLength = BLOCK_SIZE; // Always pad, even if perfectly aligned
    ...
    public byte[] removeNonAsciiPadding(byte[] input) {
        int lastGoodIndex = input.length;

        for (int i = input.length - 1; i >= 0; i--) {
            byte b = input[i];
            if (isSafeCharacter(b)) {
                lastGoodIndex = i + 1;
                break;
            }
        }

        byte[] unpadded = Arrays.copyOf(input, lastGoodIndex);
        return unpadded;
    }

    private boolean isSafeCharacter(byte b) {
        char c = (char) (b & 0xFF);
        return (c >= 'A' && c <= 'Z') ||
                (c >= 'a' && c <= 'z') ||
                (c >= '0' && c <= '9') ||
    ...
    public void doall(String[] args) {
        ...
        int option = args[0].equals("e") ? 0 : 1;

        try {
            readFile(args[3], KEY_SIZE, K1);
            readFile(args[4], KEY_SIZE, K2);
            readFile(args[5], KEY_SIZE, K3);
        } catch (IOException e) {
        ...
        long fileSize = file.length(); // returns size in bytes
        byte[] INPUT = new byte[(int) fileSize];

        try (FileInputStream fileInputStream = new FileInputStream(file)) {
            fileInputStream.read(INPUT, 0, INPUT.length);
            byte[] paddedInput = addPadding(INPUT);
            byte[] O1 = new byte[(int) paddedInput.length];
            byte[] O2 = new byte[(int) paddedInput.length];
            byte[] O3 = new byte[(int) paddedInput.length];
            if (option == 0) {
                Encryption(paddedInput, K1, O1, 0);
                Encryption(O1, K2, O2, 1);
                Encryption(O2, K3, O3, 0);
                writeFile(args[2], O3);
            } else {
                Encryption(paddedInput, K3, O1, 1);
                Encryption(O1, K2, O2, 0);
                Encryption(O2, K1, O3, 1);
                byte[] result = removeNonAsciiPadding(O3);
